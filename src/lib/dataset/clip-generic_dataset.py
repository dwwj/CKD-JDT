...
class GenericDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    opt = self.opt
    img, anns, img_info, img_path = self._load_data(index)#z读取当前帧图片

    #随机裁剪当前帧图片 
    img,anns,coord=self.crop_patch(img,anns,opt.input_w,opt.input_h)
    ...
    if opt.tracking:
      ...
      #随机裁剪前一帧图片
      pre_image,pre_anns=self.crop_pre_patch(pre_image,pre_anns,coord)
  ...
  #新增图片随机裁剪2023/09/01
  def crop_patch(self, img, labels,px,py):
        '''
        img-[H,W,C]图像矩阵
        labels-[]当前帧图片标签[{'id': 11000, 'category_id': 1, 'image_id': 5901, 'track_id': 22, 'bbox': [...], 'conf': 1.0}]
        px-opt.input_w
        py-opt.input_h
        '''
        H, W, C = img.shape
        ...
        pad_img=np.zeros((H,W,C))
        pad_img[:img.shape[0],:img.shape[1],:]=img#用0填充较小图片边缘
        # px, py = self.patch_size
        # spatch  = []
        # slabel = []
        # for i in range(img_num):
        if len(labels)==0:
          nx = np.random.randint(0,W - px+1)
          ny = np.random.randint(0,H - py+1)
          ...
          patch = pad_img[ny: ny + py, nx: nx + px, :]

          return patch, labels,patch_coord
        else:
        # print(labels)
          obj_num = len(labels)
          index = np.random.randint(0, obj_num)
          # ux, uy, vx, vy = labels[index]['bbox']
          ux=max(labels[index]['bbox'][0],0)
          uy=max(labels[index]['bbox'][1],0)
          vx=min(labels[index]['bbox'][2]+labels[index]['bbox'][0],W)
          vy=min(labels[index]['bbox'][3]+labels[index]['bbox'][1],H)
          ...
          bboxes=np.array([l['bbox'] for l in labels])
          ...
          #     continue
          patch = pad_img[ny: ny + py, nx: nx + px, :]
          # img=patch.astype(np.int8)
          #处理标签
          # label = labels[index, :]
          label=[labels[i] for i, x in enumerate(index) if x]
          if len(label)==0:#如果标签为空
              return patch, label,patch_coord
          for l in range(len(label)):
            b=label[l]['bbox']
            b[2]=b[2]+b[0]
            b[3]=b[3]+b[1]
            b[0] = np.maximum(b[0] - patch_coord[0, 0], 0)
            b[1] = np.maximum(b[1] - patch_coord[0, 1], 0)
            b[2] = np.minimum(px + b[2] - patch_coord[0, 2], px)-b[0]
            b[3] = np.minimum(py + b[3] - patch_coord[0, 3], py)-b[1]
            
            label[l]['bbox']=b
        # spatch.append(patch)
        # slabel.append(label)
        # if len(spatch)>=img_num:TODO
        #     break
        # return spatch, slabel,patch_coord
          
          #   img=cv2.rectangle(img,(int(b[0]),int(b[1])),(int(b[2]+b[0]),int(b[3]+b[1])),(0,255,0),2)
          # # cv2.imshow('img', img)
          # # cv2.waitKey(0)
          # cv2.imwrite('cur-img.png',img)
       
          return patch, label,patch_coord
  
  #新增前一帧图片随机裁剪2023/09/01
  def crop_pre_patch(self, img, labels,patch_coord):
        H, W, C = img.shape
        
        # px, py = self.patch_size
        px=patch_coord[0,2]-patch_coord[0,0]
        py=patch_coord[0,3]-patch_coord[0,1]

        W=max(px,W)
        H=max(py,H)
        pad_img=np.zeros((H,W,C))
        pad_img[:img.shape[0],:img.shape[1],:]=img
        patch = pad_img[patch_coord[0,1]: patch_coord[0,3], patch_coord[0,0]: patch_coord[0,2], :]
        # img=patch.astype(np.int8)
        if len(labels)==0:
          return patch, labels
        else:
