...
import pdb
from lib.model.SemCKD import SemCKDLoss
...
class GenericLoss(torch.nn.Module):
  ...
  def forward(self, outputs, batch):
    opt = self.opt
    losses = {head: 0 for head in opt.heads}#4种loss:hm,reg,wh,tracking
    ...
    for s in range(opt.num_stacks):
      ...
      if 'kd' in output:
        losses['kd'] += self.crit_kd(output['kd']['s_value'],output['kd']['f_target'],output['kd']['weight'])
...
class Trainer(object):
  ...
  def _get_losses(self, opt):
    loss_order = ['hm', 'wh', 'reg', 'ltrb', 'hps', 'hm_hp', \
      ...
      'ltrb_amodal', 'tracking', 'nuscenes_att', 'velocity','kd']
